...
class Core {
	...
	public function update( $data ) {
		...
		return update_post_meta( $post_id, '_grids_gutter_x', $this->update_gutter_x( $post_id, $params ) );
	}

	/**
	 * Update the horizontal gutter meta data.
	 *
	 * @since 1.0.0
	 * @param integer $post_id The post ID.
	 * @param array $params The post meta params.
	 * @return string
	 */
	private function update_gutter_x( $post_id, $params ) {
		$gutter = array();

		foreach ( $this->get_config( 'breakpoints' ) as $breakpoint => $breakpoint_data ) {
			if ( isset( $params[ 'gutter' ][ 'x' ][ $breakpoint ][ 'value' ] ) ) {
				$gutter[ $breakpoint ] = sanitize_text_field( $params[ 'gutter' ][ 'x' ][ $breakpoint ][ 'value' ] );
			}

			if ( isset( $params[ 'gutter' ][ 'x' ][ $breakpoint ][ 'unit' ] ) ) {
				$gutter[ $breakpoint . '_unit' ] = sanitize_text_field( $params[ 'gutter' ][ 'x' ][ $breakpoint ][ 'unit' ] );
			}
		}

		return json_encode( $gutter );
	}
